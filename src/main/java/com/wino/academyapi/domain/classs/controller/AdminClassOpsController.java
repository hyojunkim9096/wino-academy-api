// src/main/java/com/wino/academyapi/domain/classs/controller/AdminClassOpsController.java
package com.wino.academyapi.domain.classs.controller;

import com.wino.academyapi.domain.classs.dto.ClassOpsDtos;
import com.wino.academyapi.domain.classs.service.ClassOpsService;
 ...
 */
@RestController
@RequestMapping("/api/admin/classes/ops")
@RequiredArgsConstructor
public class AdminClassOpsController {

    private final ClassOpsService svc;

    /** 미리보기(영향 범위 집계) */
    @GetMapping("/preview")
    public ClassOpsDtos.PreviewRes preview(
    ...
    @PostMapping("/close")
    public ClassOpsDtos.SimpleAck close(
            ...
            @Validated @RequestBody ClassOpsDtos.CloseReq req
    ){
    ...
    @PostMapping("/restore")
    public ClassOpsDtos.SimpleAck restore(
            @RequestHeader(value = "X-App-User-Id",     required = false) Long   appUserId,
            @RequestHeader(value = "X-Event-Note",      required = false) String noteHeaderAscii,
            @RequestHeader(value = "X-Event-Note-B64",  required = false) String noteHeaderB64,
            @Validated @RequestBody ClassOpsDtos.RestoreReq req
    ){
        Long uid = Objects.requireNonNullElse(appUserId, 0L);
        String note = coalesceNote(req.getNote(), noteHeaderAscii, noteHeaderB64);
        req.setNote(note);
